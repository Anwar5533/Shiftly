...
import { User, UserRole } from '@prisma/client-identity-service';
...
export class AuthService {
  ...
  async sendOtp(phone: string, requestId?: string, correlationId?: string): Promise<void> {
    ...
    const enableStaticOtp = this.config.get<boolean>('app.enableStaticOtp', false);

    const otp =
      isDev && enableStaticOtp ? '123456' : Math.floor(100000 + Math.random() * 900000).toString();
  ...
  async refreshTokens(
    ...
    const hashedJti = crypto.createHash('sha256').update(payload.sessionId).digest('hex');
