...
export class ShiftsService {
  ...
  async clockIn(userId: string, shiftId: string, location: any) {
    ...
    if (shift.status !== 'SCHEDULED') {
      throw new BadRequestException(`Cannot clock into shift with status ${shift.status}`);
    }
  ...
  async clockOut(userId: string, shiftId: string, location: any, notes?: string) {
    ...
    if (shift.status !== 'IN_PROGRESS') {
      throw new BadRequestException(`Cannot clock out of shift with status ${shift.status}`);
    }
