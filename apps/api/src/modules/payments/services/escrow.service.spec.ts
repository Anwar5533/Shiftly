...
describe('EscrowService', () => {
  ...
  describe('createEscrow', () => {
    it('should throw BadRequestException if insufficient balance', async () => {
      walletService.getBalance.mockResolvedValue({ id: 'wallet-1', balance: new Decimal(50) });
      await expect(service.createEscrow('emp-1', { jobId: 'job-1', applicationId: 'app-1', amount: 100 })).rejects.toThrow(BadRequestException);
    });

    it('should deduct from wallet and create escrow', async () => {
      walletService.getBalance.mockResolvedValue({ id: 'wallet-1', balance: new Decimal(200) });
      prisma.wallet.update.mockResolvedValue({ id: 'wallet-1' });
      prisma.escrowLock.create.mockResolvedValue({ id: 'escrow-1' });

      const result = await service.createEscrow('emp-1', { jobId: 'job-1', applicationId: 'app-1', amount: 100 });
  ...
  describe('releaseEscrow', () => {
    it('should throw NotFoundException if escrow not found', async () => {
      ...
      await expect(service.releaseEscrow('emp-1', 'escrow-1')).rejects.toThrow(NotFoundException);
    });
    ...
    it('should throw BadRequestException if wrong employer', async () => {
      ...
      await expect(service.releaseEscrow('emp-1', 'escrow-1')).rejects.toThrow(BadRequestException);
    });
    ...
    it('should release escrow and add to worker wallet', async () => {
      ...
      walletService.getBalance.mockResolvedValue({ id: 'wallet-2', balance: new Decimal(0) });
