...
export class PaymentsController {
  ...
  @Post('wallet/topup')
  topupWallet(@CurrentUser('sub') userId: string, @Body() topupDto: TopupWalletDto) {
    return this.walletService.topup(userId, topupDto);
  }

  @Post('wallet/withdraw')
  withdrawFunds(@CurrentUser('sub') userId: string, @Body() withdrawDto: WithdrawFundsDto) {
    return this.walletService.withdraw(userId, withdrawDto);
  }

  @Post('escrow/create')
  @Roles(UserRole.EMPLOYER)
  createEscrow(@CurrentUser('sub') employerId: string, @Body() createEscrowDto: CreateEscrowDto) {
    return this.escrowService.createEscrow(employerId, createEscrowDto);
  }

  @Post('escrow/:id/release')
  @Roles(UserRole.EMPLOYER)
  releaseEscrow(@CurrentUser('sub') employerId: string, @Param('id') escrowId: string) {
