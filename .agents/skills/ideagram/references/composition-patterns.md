# Composition patterns & the metaphor recipe (generation path)

The single biggest lever on quality isn't rendering — it's **what you decide to depict**. A generic prompt ("illustration about analytics") yields generic art. A specific scene built on a fresh metaphor yields something that looks art-directed. This file is how you get there.
...
Use **1–2** objects, never a cluttered pile.

## Step 3 — The scene, not the subject

An amateur illustration is a subject floating on white. A designed one is a **scene** with depth. Build in layers:

1. **Background** — a calm large shape or suggested environment (a soft-tinted panel, a wall, a horizon line, a window). Often the biggest element, lowest contrast.
2. **Midground** — the metaphor object / device the concept is about. Carries the accent.
3. **Foreground figure** — the character performing the action, interacting with the midground.
4. **Grounding + a few decorative beats** — a soft shadow under the subject, one or two small accent details (a plant, floating cards, a couple of dots) to add life. Sparse.
...
- One focal point, accent-colored. Everything else is neutral.
- ≤ 3 short text labels, and only if essential. Prefer zero.
- One or two objects, not a set. Whitespace over density.
- Don't explain every element — suggestion reads as confident; over-labeling reads as a diagram.
