...
3. **Figure(s)** — often _smaller_ than you'd expect, positioned relative to the background element, interacting with it.
...
The old approach hand-built figures from circles + capsules + two colors → pictograms with no finesse, no scene, no depth. This approach reuses the actual illustrator's vector work → genuine unDraw quality, because it _is_ unDraw. The skill's value moves from "drawing" (which it's bad at) to "matching + recoloring + composing" (which it's reliable at).
