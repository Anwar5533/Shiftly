...
`design-brief` to frame it, then `/design-research:discover` for personas and journeys. If you're arguing about what the product is _for_, `north-star-vision` first.
...
`form-design` if it's a form, `onboarding-design` if it's first-run, `error-handling-ux` if they're hitting errors. `hicks-law` and `millers-law` explain _why_ a step is too heavy.
