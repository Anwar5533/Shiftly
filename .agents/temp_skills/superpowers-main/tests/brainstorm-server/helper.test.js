...
const { nextReconnectDelay, MIN_RECONNECT_MS, MAX_RECONNECT_MS, TOMBSTONE_AFTER_MS } =
  moduleShim.exports;
...
test('reflects all three connection states', () => {
  assert(
    /Connected/.test(src) && /Reconnecting/.test(src) && /Disconnected/.test(src),
  ...
  const env = {
    ...
    window: {
      location: {
        ...
        replace(url) {
          state.replacements.push(url);
        },
      },
      sessionStorage: {
        getItem: (key) => (key === 'brainstorm-session-key' ? state.sessionKey : null),
      },
    },
    document: {
    ...
    WebSocket: FakeWS,
    setTimeout: (fn, ms) => {
      state.timers.push({ fn, ms, fired: false, cleared: false });
      return state.timers.length;
    },
    clearTimeout: (id) => {
      if (state.timers[id - 1]) state.timers[id - 1].cleared = true;
    },
    Date: { now: () => state.now },
    console,
  };
...
test('rebootstraps with stored key when a tombstoned connection comes back', () => {
  ...
  e.advance(20000);
  e.fireReconnect();
  e.last().close(); // tombstone now shown
  assert.deepStrictEqual(e.state.replacements, []);
  e.fireReconnect();
  e.last().open(); // server back (e.g. same-port restart)
...
test('reloads to recover when tombstoned and no sessionStorage key is present', () => {
  ...
  e.fireReconnect();
  e.last().open(); // server back (e.g. cookie-only page)
