...
- Added a new anti-slop gate checking that section padding is actually generous and weight-tiered, distinct from the existing mechanism-consistency gate (a page can use one consistent separation _mechanism_ and still fail this if that mechanism is applied at a cramped value everywhere).
